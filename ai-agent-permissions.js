(function () {
  ...
  /*
    SherGuard AI Agent Permissions
    Frontend policy controller.

    Fix included:
    - Global Auto Action now synchronizes module Auto Action toggles.
    - Module localStorage preference keys are updated safely.
    - Dashboard config window.aiAgentConfig is refreshed.
    - Modules are notified with events so they can re-render.
    - Local policy still remains available for dashboard/frontend enforcement.
  */
  ...
  var moduleAutoActionMap = {
    ...
    'Device Risk': {
      ...
      storageKey: aiTrustScopedKey('aiTrustOsDeviceRiskAutoAction')
    },
    'Bot Detection': {
      toggleId: 'botAutoActionToggle',
      storageKey: aiTrustScopedKey('aiTrustOsBotAutoAction')
    },
    'API Abuse': {
      toggleId: 'apiAutoActionToggle',
      storageKey: aiTrustScopedKey('aiTrustOsApiAutoAction')
    },
    'Payment Fraud': {
      toggleId: 'paymentAutoActionToggle',
      storageKey: aiTrustScopedKey('aiTrustOsPaymentFraudAutoAction')
    }
  };
  ...
  function normalizePolicyModules() {
    ...
    defaultModules.forEach(function (moduleName) {
      ...
      state.modules[moduleName].read =
        state.modules[moduleName].read !== false;

      state.modules[moduleName].write =
        state.modules[moduleName].write === true;

      state.modules[moduleName].execute =
        state.modules[moduleName].execute === true;

      state.modules[moduleName].autoAction =
        state.modules[moduleName].autoAction === true;

      state.modules[moduleName].approvalRequired =
        state.modules[moduleName].approvalRequired !== false;
    });
  }
  ...
  function loadPolicy() {
    ...
    normalizePolicyModules();
    syncWindowConfig();
  }

  function syncWindowConfig() {
    window.aiAgentConfig = {
      masterEnabled: state.masterAI === true,
      strictMode: state.strictMode === true,
      autoAction: state.autoAction === true,
      approvalRequired: state.approvalRequired !== false,
  ...
  function syncModuleAutoActions() {
    defaultModules.forEach(function (moduleName) {
      var modulePolicy = state.modules[moduleName];
      var mapping = moduleAutoActionMap[moduleName];

      if (!modulePolicy || !mapping) {
        return;
      }
      ...
      var toggle = document.getElementById(mapping.toggleId);
    ...
    window.dispatchEvent(
      new CustomEvent('sherguardAiPolicyUpdated', {
        detail: {
          policy: state,
          config: window.aiAgentConfig,
  ...
  function savePolicy() {
    ...
    syncModuleAutoActions();
  }

  function getAllowedActions() {
    if (!state.masterAI) {
      return [
        'Read-only dashboard suggestions',
  ...
  function getRestrictedActions() {
    if (state.strictMode) {
      return [
        'Policy override',
        'High-risk module changes without approval',
        'Unsafe automation outside enabled modules'
      ];
    }
    ...
    return [
      'Sensitive module writes',
      'Auto action without module permission',
  ...
  function getBlockedActions() {
    ...
    if (!state.masterAI) {
      return [
        'All AI write actions',
        'All AI execute actions',
        'All AI auto actions'
      ];
    }
  ...
  function renderActivityLog() {
    ...
    if (state.activity.length === 0) {
      activityBody.innerHTML =
        '<tr><td colspan="6" class="ai-agent-empty-row">No activity yet.</td></tr>';
      return;
    }
  ...
  function applyGlobalPolicyToModules() {
    Object.keys(state.modules).forEach(function (moduleName) {
      if (!state.modules[moduleName]) {
        return;
      }
  ...
  function applyPermissions() {
    ...
    addActivity(
      'Apply Permissions',
      'Global Policy',
      'Policy settings applied and module auto actions synchronized',
      'Manual',
  ...
  function resetPolicies() {
    ...
    addActivity(
      'Reset Policies',
      'Global Policy',
      'Default policy restored and module automation disabled',
      'Manual',
      'Success'
    );
  ...
  function handleMatrixChange(event) {
    ...
    if (permissionKey === 'autoAction') {
      var mapping = moduleAutoActionMap[moduleName];

      if (mapping) {
        var moduleAutoAllowed =
          state.masterAI === true &&
          state.autoAction === true &&
          target.checked === true;
        ...
        try {
          localStorage.setItem(
            mapping.storageKey,
            moduleAutoAllowed ? 'true' : 'false'
          );
        } catch (error) {}

        var moduleToggle = document.getElementById(mapping.toggleId);

        if (moduleToggle) {
          moduleToggle.checked = moduleAutoAllowed;
        }
      }
    }
  ...
  window.SherGuardAiAgentPermissions = {
    getPolicy: function () {
      return state;
    },
