      ...
      return records
        ...
        .map(function (item) {
          ...
          return {
            ...
            breakdown: Array.isArray(item.reasons)
            ? item.reasons.map(function (reason, index) {
                return {
                  code: 'backend-email-risk-' + index,
                  title: reason,
                  group:
                    String(reason).toLowerCase().includes('domain')
                      ? 'provider'
                      : String(reason).toLowerCase().includes('username') ||
                        String(reason).toLowerCase().includes('synthetic') ||
                        String(reason).toLowerCase().includes('random')
                      ? 'pattern'
                      : 'engine',
                  severity:
                    riskLabel === 'High Risk'
          ...
          signals: buildSignals(
            ...
            },
            {
              breakdown: Array.isArray(item.reasons)
                ...
                : [],
              patternRuleCount: Array.isArray(item.reasons)
                ? item.reasons.length
                : 0
            }
          ),
...
function buildDomainReputation(result) {
  const riskLabel = result && result.riskLabel
    ? result.riskLabel
    : '';

  const riskReasons = Array.isArray(result && result.riskReasons)
    ? result.riskReasons.join(' ').toLowerCase()
    : '';

  if (
    riskLabel === 'High Risk' &&
    (
      riskReasons.includes('random') ||
      riskReasons.includes('synthetic') ||
      riskReasons.includes('unknown custom domain')
    )
  ) {
    return {
      level: 'Poor',
      trustTier: 'Suspicious Custom Domain',
      explanation: 'This email uses random-looking identity signals or an unknown custom domain pattern. Extra verification or blocking is recommended.'
    };
  }
  ...
  if (result.providerType === 'business') {
    return {
      ...
      explanation: 'Custom business domains usually provide stronger identity ownership signals when no high-risk identity pattern is detected.'
    };
  }
