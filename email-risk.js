...
renderBreakdown(result.breakdown || []);
    ...
    function updateKpis() {
      ...
      const high = state.activity.filter((item) => item.riskLabel === 'High Risk').length;
      const medium = state.activity.filter((item) => item.riskLabel === 'Medium Risk').length;
      const low = state.activity.filter((item) => item.riskLabel === 'Low Risk').length;
  ...
  ? Math.round(
      state.activity.reduce((sum, item) => sum + Number(item.score || 0), 0) / total
    )
  : 0;
...
setText(dom.insightText, insightMessage);
    ...
    function getFilteredActivity() {
      ...
      let records = state.activity.slice();
